...
const Terrain = memo(function Terrain({ setTerrainGeo }) {
  ...
  const { obstacles, borderMountains } = useMemo(() => {
    ...
    for (let i = -2; i <= 2; i++) {
      obs.push({ x: dinoX + Math.sin(angle) * (i * 12), z: dinoZ + Math.cos(angle) * (i * 12), radius: 14, type: 'dinosaur' });
    }
    ...
    const addEdge = (startX, startZ, endX, endZ, type = 'mountain', skipChance = 0) => {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        mts.push({ pos: [jx, y, jz], rot: [0, Math.random() * Math.PI, 0], scale: elementScale, type: type });
        // Register the mountain as an obstacle so trees avoid it
        obs.push({ x: jx, z: jz, radius: elementScale * 0.75, type: 'mountain' });
      }
    };
    ...
    return { obstacles: obs, borderMountains: mts };
  }, []); // Run once
  ...
  return (
    <group>
      ...
      <BorderMountains mountains={borderMountains} />
      <Ocean />
