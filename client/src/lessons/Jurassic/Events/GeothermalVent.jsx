...
// 1. Define the Procedural Steam Shader with FOG SUPPORT
const SteamShaderMaterial = shaderMaterial(
  { 
    uTime: 0, 
    uOpacity: 0.5, 
    uColor: new THREE.Color('#ffffff'),
    // REQUIRED FOG UNIFORMS: Prevents the "refreshFogUniforms" crash
  ...
  `
    ...
    // Fractional Brownian Motion for the cloud billows
    float fbm (vec2 st) {
        float value = 0.0;
        float amplitude = 0.5;
        for (int i = 0; i < 5; i++) {
    ...
    void main() {
        ...
        // Soft circular mask so the plane edges are invisible
        float dist = distance(vUv, vec2(0.5));
        float mask = smoothstep(0.5, 0.1, dist);
        
        // Final alpha combines the noise texture, the circular mask, and base opacity
        float alpha = n * mask * uOpacity;
        
        gl_FragColor = vec4(uColor, alpha);

        #include <fog_fragment> // Apply scene fog to the final color
    }
  `
);

// Register it with React Three Fiber
...
export default function GeothermalVent({ x, y, z, scale = 1 }) {
    ...
    useFrame((state) => {
        ...
        // Offset the time slightly for each layer to create depth
