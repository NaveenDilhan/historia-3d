...
export default function TimelineHUD({ activeEra, progress, visible }) {
  return (
    ...
    >
      ...
      <div className="w-96 h-2 bg-gray-800 rounded-full mt-4 overflow-hidden border border-white/10 shadow-[0_0_15px_rgba(0,0,0,0.5)]">
        <div 
          className="h-full bg-gradient-to-r from-emerald-500 to-cyan-400 transition-all duration-300 ease-out"
          style={{ width: `${progress * 100}%` }}
        />
      </div>
      <p className="text-white/50 text-xs mt-2 font-mono uppercase tracking-widest">Scroll to advance time</p>
    </div>
  );
}